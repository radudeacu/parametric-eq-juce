...
void HighBand::prepare (const juce::dsp::ProcessSpec& spec)
{
...
void HighBand::setParameters (HighBandType type, float freqHz, float gainDb, float q, int slopeDbPerOctave,
                               int numSamples)
{
    if (firstUpdate)
    {
...
void HighBand::process (juce::dsp::ProcessContextReplacing<float>& context)
{
